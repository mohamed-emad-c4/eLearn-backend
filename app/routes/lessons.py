...
async def create_lesson(lesson: schemas.LessonCreate, db: AsyncSession = Depends(get_db), instructor=Depends(auth.get_current_instructor)):
    new_lesson = models.Lesson(title=lesson.title, video_url=lesson.video_url, content=lesson.content, resource_links=lesson.resource_links, order_number=lesson.order_number, chapter_id=lesson.chapter_id)
    db.add(new_lesson)
    await db.commit()
